...
next
  ...
  obtain  ts_c ts'' where l_func_t: "length (func_t \<C>) > j"
                                    "ts = ts_c @ ts'' \<and> (return \<C>) = Some ts''"
    print_statement return_call
    sorry
...
proof -
  have "\<And>es. Lfilled 0 (LBase cs es_c) es (cs@es@es_c)"
    ...
    by fastforce
  thus ?thesis
    using reduce.intros(26) assms(1)
    by blast
qed
...
next
  ...
  hence "vs = vj @ [v] @ vj'"
    ...
    by fastforce
  thus ?case
    using progress_L0[OF reduce.intros(11)[OF vj_len, of s v vj'] get_local(6)]
    by fastforce
next
  ...
  hence "vs = vj @ [v] @ vj'"
    ...
    by fastforce
  thus ?case
    using reduce.intros(12)[OF vj_len, of s v vj' v' i] cs_def
    by fastforce
next
...
next
  ...
  thus ?case
    using reduce.intros(13)[of s vs j i] progress_L0
    by fastforce
next
  ...
  obtain v where "cs = [$C v]"
    ...
    by fastforce
  thus ?case
    using reduce.intros(14)[of s i j v _ vs]
    by fastforce
next
  ...
  proof (cases tp_sx)
    ...
    proof (cases "load ((mem s)!j) (nat_of_int c) off (t_length t)")
      case None
      show ?thesis
        using reduce.intros(16)[OF mem_some _ None, of vs] tp_none load(2)
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(15)[OF mem_some _ Some, of vs] tp_none load(2)
        by fastforce
    qed
  next
    case (Some a)
    ...
    proof (cases "load_packed sx ((mem s)!j) (nat_of_int c) off (tp_length tp) (t_length t)")
      case None
      show ?thesis
        using reduce.intros(18)[OF mem_some _ None, of vs] tp_some load(2)
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(17)[OF mem_some _ Some, of vs] tp_some load(2)
        by fastforce
    qed
  qed
  then show ?case
...
next
  ...
  proof (cases tp)
    ...
    proof (cases "store (s.mem s ! j) (nat_of_int c) off (bits v) (t_length t)")
      case None
      show ?thesis
        using reduce.intros(20)[OF _ mem_some _ None, of vs] t_def tp_none store(2)
        unfolding types_agree_def
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(19)[OF _ mem_some _ Some, of vs] t_def tp_none store(2)
        unfolding types_agree_def
        by fastforce
    qed
  next
    ...
    proof (cases "store_packed (s.mem s ! j) (nat_of_int c) off (bits v) (tp_length a)")
      case None
      show ?thesis
        using reduce.intros(22)[OF _ mem_some _ None, of t vs] t_def tp_some store(2)
        unfolding types_agree_def
        by fastforce
    next
      case (Some a)
      show ?thesis
        using reduce.intros(21)[OF _ mem_some _ Some, of t vs] t_def tp_some store(2)
        unfolding types_agree_def
        by fastforce
    qed
  qed
...
next
  ...
  obtain j where mem_some:"smem_ind s i = Some j"
    ...
    by fastforce
  thus ?case
    using progress_L0[OF reduce.intros(23)[OF mem_some] current_memory(5), of _ _ vs "[]"]
    by fastforce
next
  ...
  obtain j where mem_some:"smem_ind s i = Some j"
    ...
    by fastforce
  show ?case
    using reduce.intros(25)[OF mem_some, of _] c_def
    by fastforce
next
  case (empty \<C>)
  thus ?case
...
lemma progress_e:
  assumes "\<S>\<bullet>None \<tturnstile>_i vs;cs_es : ts'"
          ...
          "\<And> i k lholed. (Lfilled k lholed [$ReturnCall (i)] cs_es) \<Longrightarrow> i < k"
          "cs_es \<noteq> [Trap]"
...
proof -
  ...
  have prems1:
      "\<S>\<bullet>\<C> \<turnstile> es : (ts_c _> ts') \<Longrightarrow>
       ...
       (\<And> i k lholed. (Lfilled k lholed [$ReturnCall (i)] cs_es) \<Longrightarrow> i < k) \<Longrightarrow>
  ...
  next
    ...
    next
      ...
      qed
      moreover
      have "\<And>i k lholed.  Lfilled k lholed [$ReturnCall i] (cs @ es) \<Longrightarrow> i < k"
      proof -
        {
          assume "\<exists>i k lholed.  Lfilled k lholed [$ReturnCall i] (cs @ es) \<and> \<not>(i < k)"
          then obtain i k lholed where local_assms:"Lfilled k lholed [$ReturnCall i] (cs @ es)" "\<not>(i < k)"
            by blast
          hence "\<exists>lholed'. Lfilled k lholed' [$ReturnCall i] (cs @ es @ [e]) \<and> \<not>(i < k)"
          proof (cases rule: Lfilled.cases)
            case (L0 vs es')
            obtain lholed' where "lholed' = LBase vs (es'@[e])"
              by blast
        ...
        thus "\<And>i k lholed.  Lfilled k lholed [$ReturnCall i] (cs @ es) \<Longrightarrow> i < k"
          by blast
      qed
      moreover
  ...
  case (5 \<S> ts j vls es n \<C>)
    consider (1) "(\<And>k lholed. \<not> Lfilled k lholed [$Return] es)"
                 ...
                 "(\<And>k lholed i. (Lfilled k lholed [$ReturnCall i] es) \<Longrightarrow> i < k)"
                 "es \<noteq> [Trap]"
                 "\<not> const_list es"
           ...
           | (5) "\<exists>k lholed i. (Lfilled k lholed [$ReturnCall i] es) \<and> i \<ge> k"
  ...
  next
    ...
    proof (cases)
      ...
      have temp6:"Lfilled 1 (LRec cs n e0s (LBase [] []) []) a (cs@[Label n e0s a])"
        ...
        by fastforce
      show ?thesis
        using reduce.intros(26)[OF _ temp5 temp6] 7(7) red_def
        by fastforce
    next
      case 2
...
lemma progress_e3:
  assumes "\<S>\<bullet>None \<tturnstile>_i vs;es : ts"
  shows "(Lfilled k lholed [$ReturnCall (j)] es) \<Longrightarrow> j < k"
proof -
  {
    assume "(\<exists>i k lholed. (Lfilled k lholed [$ReturnCall (i)] es) \<and> i \<ge> k)"
    then obtain j k lholed where local_assms:"(Lfilled k lholed [$ReturnCall (k+j)] es)"
      by (metis le_iff_add)
    obtain \<C> where c_def:"i < length (s_inst \<S>)"
                   "\<C> = ((s_inst \<S>)!i)\<lparr>local := (local ((s_inst \<S>)!i)) @ (map typeof vs), return := None\<rparr>"
                   "(\<S>\<bullet>\<C> \<turnstile> es : ([] _> ts))"
  ...
  thus "(\<And> j k lholed. (Lfilled k lholed [$ReturnCall (j)] es) \<Longrightarrow> j < k)"
    by fastforce
qed
