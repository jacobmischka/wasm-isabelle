...
next
  ...
  thus ?thesis
    using assms
    by (cases "cvt_sat t2 sx a"; cases "types_agree t1 a") auto
next
  fix a ves'
...
lemma run_one_step_label_return_call_imp_return:
  assumes "run_one_step d i (s, vs, ves, Label n les es) = (s', vs', RSReturnCall c res)"
  shows "run_step d i (s, vs, es) = (s', vs', RSReturnCall c res)"
  using assms
proof (cases "es = [Trap]"; cases "const_list es")
  assume local_assms:"es \<noteq> [Trap]" "\<not>const_list es"
  obtain s'' vs'' res'' where rs_def:"run_step d i (s, vs, es) = (s'', vs'', res'')"
...
proof -
  ...
  proof (induction d i "(s,vs,es)" and d i "(s,vs,ves,e)" arbitrary: s vs es s' vs' c res and s vs ves e s' vs' c res rule: run_step_run_one_step.induct)
    ...
    obtain s'' vs'' res'' where "run_one_step d i (s,vs,(rev ves),a) = (s'', vs'', res'')"
      by (metis surj_pair)
    hence ros_def:"run_one_step d i (s,vs,(rev ves),a) = (s', vs', RSReturnCall c res)"
      using 1(2) split_vals_es es'_def a_def
      by (cases "res''") (auto simp del: run_one_step.simps)
    obtain n lfilled j les_c les n' les' where
      "s = s'" "vs = vs'"
      "(res = rev ves \<and> a = $ReturnCall j) \<or> (Lfilled_exact n lfilled ((vs_to_es res) @ [$ReturnCall j] @ les_c) les \<and> a = Label n' les' les)"
      using 1(1)[OF split_vals_es[symmetric] _ es'_def a_def ros_def]
      by fastforce
    then consider
      (1) "s = s'" "vs = vs'" "res = rev ves" "a = $ReturnCall j"
    | (2) "s = s'" "vs = vs'" "(Lfilled_exact n lfilled ((vs_to_es res) @ [$ReturnCall j] @ les_c) les)" "(a = Label n' les' les)"
      by blast
    thus ?case
    proof cases
      case 1
  ...
  next
    ...
    next
      ...
      hence "run_step d i (s, vs, es) = (s', vs', RSReturnCall c res)"
        ...
        by fastforce
      thus ?thesis
        using 2(3) 2(1)[OF e_def _ run_step_return_call_imp_not_trap_const_list(2)] e_def
        by fastforce
    qed
  qed
  thus ?thesis
      ...
      proof (cases "length ves \<ge> n")
        ...
        proof (cases cl)
          ...
          have "\<lparr>s;vs;(vs_to_es ves')@[Callcl cl]\<rparr> \<leadsto>_i \<lparr>s;vs;([Local (length t2s) i' (rev ves' @ (n_zeros fts)) [$Block ([] _> t2s) fes]])\<rparr>"
            using reduce.intros(8) local_defs(1,2) Func_native ves'_length
            unfolding cl_type_def
            by fastforce
          ultimately
          show ?thesis
          ...
          proof (cases "host_apply_impl s (t1s _> t2s) x22 (rev ves')")
            ...
            thus ?thesis
              using is_const_list[of _ "(rev ves'')"]
                    reduce.intros(10)[OF _ _ ves'_length local_defs(2)]
            ...
            case (Pair rs rves)
              ...
              proof (cases "list_all2 types_agree t2s rves")
                ...
                hence "rs = s'"
                  ...
                  by auto
                thus ?thesis
                  using progress_L0_left reduce.intros(9)[OF _ _ ves'_length local_defs(2)] Pair
            ...
            next
              ...
              thus ?thesis
                using Local 2(3) Local outer_outer_false False run_step_is Suc
                      reduce.intros(27)[OF inner_reduce] RSNormal
